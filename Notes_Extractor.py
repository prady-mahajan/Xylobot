...
file_name =  "st.wav"# put the path here
...
def not_redundant(start, end):
    for i in sound[start:end]:
        if i != 0:
            return True
    return False
...
timesense = (file_length/f_s)
for i in range(file_length):
    wdata = audio_file.readframes(1)
    data = struct.unpack("<h", wdata)
    sound[i] = int(data[0])

sound_original = copy.deepcopy(sound)

sound = np.divide(sound, float(2**28))  # scaling it to 0 - 1
...
for i in range(len(sound)):
    if vacation:
        if sound[i] == 0:
            ...
            vacation = futurelook(i)
            if vacation:
                continue
            else:
                end = i
                if not_redundant(start, end):
...
bucket.append((start, len(sound)))

sound = sound_original

for j in bucket:
 # peak detection
    notelist = sound[j[0]:j[1]]
    fourier = np.fft.fft(notelist)
    ...
    freq = freq*(len(sound_original)/(j[1]-j[0]))
    ...
    lines.append(note+"-"+f"{round(timesense*j[0]/len(sound_original), 1)}")

lines.append("END-"+f"{round(timesense, 1)}")
